...
namespace Chat_Server
{
    ...
    class NetworkService
    {
        ...
        public void start(string host, int  port, int backlog)
        {
            ...
            roomManager_cs.set_chatManager(chatManager_cs);
            UsersCollection = database.GetCollection<BsonDocument>(Users_Collection);
            try
            {
        ...
        void ReceiveCompleted(object sender, SocketAsyncEventArgs e)
        {
            ...
            try
            {
                ...
                if (bytesReceived > 0 && e.SocketError == SocketError.Success)
                {
                    ...
                    {
                        var filter = Builders<BsonDocument>.Filter.Eq("ID", receivedInfo.info.ID);
                        var docs = UsersCollection.Find(filter).ToList();
                        if (docs.Count == 0)
                        {
                            var doc = new BsonDocument { { "ID", receivedInfo.info.ID }, { "PW", receivedInfo.info.PW }, { "LoginST", 0 } };
                    ...
                    {
                        var filter = Builders<BsonDocument>.Filter.And(Builders<BsonDocument>.Filter.Eq("ID", receivedInfo.info.ID),Builders<BsonDocument>.Filter.Eq("PW", receivedInfo.info.PW));
                        var docs = UsersCollection.Find(filter).ToList();
                        if (docs.Count == 1)
                        {
                            ...
                            if (loginST == 0)
                            {
                                var updateFilter = Builders<BsonDocument>.Filter.Eq("ID", firstDoc["ID"]); // 특정 문서를 찾기 위한 필터
        ...
        public List<string> deliver_user_db(Token client_token)
        {
            ...
            {
                if (!doc["ID"].AsString.Equals(client_token.client_ID)) all_users_IDs.Add(doc["ID"].AsString);
            }
        ...
        void on_session_closed(Token token)
        {
            ...
            lock (users)
            {
                ...
                var updateFilter = Builders<BsonDocument>.Filter.Eq("ID", token.client_ID); // 특정 문서를 찾기 위한 필터
                var update = Builders<BsonDocument>.Update.Set("LoginST", 0); // 새로운 LoginST 값 설정
                var updateResult = UsersCollection.UpdateOne(updateFilter, update);
